...
6. The route maps those objects into LSP types from `protocol/types.nim`, which `wrapRpc` serializes with the `LspConv` json flavor before json-rpc writes the response back to the client.
...
```bash
nimble build
./nimlangserver --mcp --socket 2>mcp.stderr.log
```
